...
def assert_successful_run(run_id: str, run_status: dict[str, Any], diagnostic: dict[str, Any]) -> None:
    ...
    if status != "completed":
        error_summary = run_status.get("errorMessage")
        raise RuntimeError(
            f"Agent run {run_id} 未成功完成: status={status}, errorSummary={error_summary!r}"
        )

    app_status = str(diagnostic.get("appStatus", "unknown"))
    if app_status != "completed":
        raise RuntimeError(
            f"Agent run {run_id} 的 Temporal 诊断应用状态异常: appStatus={app_status}"
        )
...
def main() -> int:
    ...
    with build_client(timeout_seconds=20.0) as client:
        run = read_json(client.post(
            "/api/v1/agent-runs",
            json={
                "jdText": "Need AI agent engineer with Python, ReAct, workflow orchestration, and Temporal visibility awareness",
                "sourcingPreferenceText": "Prefer retrieval, search systems, and shortlist reasoning experience",
            },
        ))

        run_status = wait_for_agent_run(client, str(run["runId"]))
        diagnostic = wait_for_agent_temporal_diagnostic(client, str(run["runId"]))
