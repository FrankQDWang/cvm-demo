# MVP Slice 02 JD KW Agent Run Plan

Use this plan for the Slice 02 mainline closure that takes the current JD / keyword extraction baseline and the bounded `AgentRun` loop, then turns it into a fully governed, test-backed mainline slice without pulling list/detail/verdict, export/audit/ops, or harness backlog into scope.

## Goal

Recruiters can provide JD intent, trigger a bounded `AgentRun`, and execute a recoverable CTS-backed search loop whose state, pagination, retries, and stop semantics are contract-backed, idempotent, and regression-tested end to end.
...
- If a requirement depends on async AI degradation beyond the bounded `AgentRun`, leave it for Slice 03 or Slice 05 and call it out in notes.
...
- Required repo state: bounded `AgentRunWorkflow`, CTS adapter, idempotency constraints, and CTS anomaly distinction already exist in the current repo.
...
- Blocking unknowns: none. The current gaps are explicit in traceability: JD archive and unsaved-change guards, long-JD contract handling, explicit `AgentRun` control semantics, and stronger retry/stop proofs.
...
2. Upgrade keyword extraction and strategy authoring: preserve evidence, support explicit operator review when needed, and keep the two-input `AgentRun` trigger as the only mainline entrypoint.
3. Complete `AgentRun` control semantics: freeze actor/version metadata, enforce the round fetch schedule and explicit stop rules, preserve partial-success visibility, and guarantee retry/idempotency behavior without duplicate resumes.
...
- Manual: user-web mainline walkthrough for `JD + 寻访偏好`, `AgentRun` start, and partial-results visibility
...
- The slice fails if `AgentRun` behavior depends on undocumented implicit UI behavior instead of explicit API and workflow rules.
...
- Risk: JD, keyword, and `AgentRun` rules are tightly coupled, so shallow fixes can create regressions across API, workflow, and user web.
