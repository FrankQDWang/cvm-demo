...
def test_execute_search_run_delegates_to_platform_service(monkeypatch) -> None:
    ...
    class FakeService:
        def execute_search_run(self, run_id: str):
            calls["run_id"] = run_id
    ...
    result = execute_search_run("run_123")

    assert result == "completed"
    assert calls == {"run_id": "run_123", "closed": True}


def test_search_run_workflow_invokes_activity(monkeypatch) -> None:
    ...
    result = asyncio.run(SearchRunWorkflow().run("run_456"))

    assert result == "completed"
    assert captured["run_id"] == "run_456"
...
def test_run_worker_builds_and_runs_temporal_worker(monkeypatch) -> None:
    ...
    assert calls["task_queue"] == "cvm-search-runs"
